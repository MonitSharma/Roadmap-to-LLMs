...
class BilingualDataset(Dataset):
    def __init__(self, ds, tokenizer_src, tokenizer_tgt, src_lang, tgt_lang, seq_len) -> None:
        ...
        self.bos_token = torch.tensor([tokenizer_src.token_to_id("[BOS]")], dtype=torch.long)
    ...
    def __getitem__(self,index:Any) -> Any:
        ...
        if enc_num_padding_tokens < 0 or dec_num_padding_tokens < 0:
            raise ValueError(f"Sequence length {self.seq_len} is too small for the input text: {src_text}")
        ...
        return {
            ...
            "decoder_mask": (decoder_input != self.pad_token).unsqueeze(0).unsqueeze(0).int() & causal_mask(self.seq_len).int(),
            "label": label,
            "src_text": src_text,
            "tgt_text": tgt_text
        }
